...
function EmployerDashboard() {
  ...
  useEffect(() => {
    const texts = [
      'VouchSafe', 'Main Menu', 'Dashboard', 'Search Employees', 'Log out',
      'Welcome back', 'Welcome to dashboard', 'Total Jobs', 'All requests', 'Pending',
      'Awaiting response', 'Active', 'In progress', 'Completed', 'Jobs finished',
      'Job Requests', 'See all', 'No jobs yet', 'Start searching for employees',
      'Rate this employee', 'Rating', 'Optional comment', 'Share your experience...',
      'Submit', 'Cancel'
    ];
        ...
        <div className="p-8">
          ...
          <div className="grid md:grid-cols-4 gap-4 mb-8">
            ...
            <div className="bg-gradient-to-br from-teal-500 to-cyan-500 rounded-2xl p-5 text-white">
              <div className="flex items-start justify-between mb-3">
                <p className="text-sm font-medium">{tSync('Completed')}</p>
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                </svg>
              </div>
              <p className="text-3xl font-bold mb-1">{completedJobs}</p>
              <p className="text-xs opacity-90">{tSync('Jobs finished')}</p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-6">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xl font-bold text-gray-900">{tSync('Job Requests')}</h3>
            ...
            <div className="space-y-3">
              {data.sent && data.sent.length > 0 ? (
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-500 mb-4">{tSync('No jobs yet')}</p>
                  <button
                    onClick={() => navigate('/search-employees')}
                    className="text-teal-600 hover:text-teal-700 font-semibold"
                  >
                    {tSync('Start searching for employees')}
                  </button>
                </div>
              )}
            </div>
